...
def start_keep_alive_thread():
    while True:
        keep_alive()
        time.sleep(30)  # Sleep for 30 seconds (30 seconds) before the next ping
...
async def run_attack_command_on_codespace(target_ip, target_port, duration, chat_id):
    ...
    try:
        ...
        output = stdout.decode().replace("", "@INFLICT_BGMI")  # Replace with your name
        error = stderr.decode()
...
def send_owner_info(message):
    owner_message = "This Bot Has Been Developed By @INFLICT_BGMI"  
...
def attack_button_handler(message):
    ...
    if not found_user:
        bot.send_message(chat_id, "*You are not registered. Please redeem A key To Owner:- @INFLICT_BGMI*", parse_mode='Markdown')
        return

    # Check if the user's key is still valid
    valid_until = datetime.fromisoformat(found_user['valid_until'])
    if datetime.now() > valid_until:
        bot.send_message(chat_id, "*Your key has expired. Please redeem A key To Owner:- @INFLICT_BGMI.*", parse_mode='Markdown')
        return
...
def my_account(message):
    ...
    if found_user:
        ...
        account_info = "Please redeem A key To Owner:- @INFLICT_BGMI."
